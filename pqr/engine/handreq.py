...
■ 이 PC 에서 Cowork(또는 Claude Code 데스크톱·터미널)를 쓰신다면 — 올릴 것 없이 그 자리에서
  이 폴더를 열고 아래 한 줄만 시키세요. 그 Claude 는 이 PC 안의 파일을 바로 읽습니다.

    아래 폴더의 13 폴더에 있는 안정성 시험일지를 모두 읽어
    '13. 안정성시험일지 판독.json' 을 만들어 이 폴더에 저장해 줘.
    형식은 이 파일 아래쪽 '판독 파일의 꼴' 그대로, covers_all 은 true 로.
    %(folder)s

■ 웹 대화(claude.ai)를 쓰신다면 — 그 대화는 회사 밖에서 돌아 이 PC 를 볼 수 없으므로,
  이 묶음(zip)을 대화창에 올리고 아래처럼 부탁하세요.
  ...
  "logs": [
    {
      "lot": "OEX101",              // 제조번호
      "year": "2024",               // 제조 연도
      "kind": "장기",                // "장기"(파일 이름 [LT-…]) 또는 "시판후"(파일 이름 [OG-…])
      "market": "내수",              // "내수" 또는 "수출"
      "pack": "5g tube/갑",
      "store": "25±2°C,\\n60±5%%RH",
      "mfg": "2024.01.10",
