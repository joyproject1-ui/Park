...
def apply(document, log=None, product_title=None, edms=None):
    ...
    # 13.3 관리 규격 줄은 어느 경로로 채웠든 **굵은 글씨** — 담당자 2026-09-10 퀴노비드: "관리 규격은 굵은
    # 글씨로 작성해줘" (앞서의 "굵게 처리 하지 않음" 을 뒤집음). 라벨 칸까지 모두 굵게.
    bold = 0
    for t in T:
        ...
        for ri, row in enumerate(t.rows):
            if re.sub(r"\s+", "", E.cell_text(E.raw_cells(row)[0])).startswith("관리규격"):
                bold += E.bold_row(t, ri, first_col=0)
    log("13.3 관리 규격 굵은 글씨: %d" % bold)
